...
class Global(object):
    def __init__(self,
        ...
        self.args = args  # args 저장 (feature_re_train 등에서 사용)
        self.fedavg_acc = []
        self.fedavg_many = []
        self.fedavg_medium = []
        self.fedavg_few = []
    ...
    def update_feature_syn(self, args, global_params, list_clients_gradient, new_text_features):
        ...
        for gradient_one in list_clients_gradient:
            for class_num, gradient in gradient_one.items():
                if class_num < self.num_classes:  # 범위 검증
    ...
    def feature_re_train(self, fedavg_params, batch_size_local_training):
        ...
        optimizer_ft_net = SGD(ft_model.parameters(), lr=self.args.lr_net)  # optimizer_img for synthetic data
        ft_model.train()
...
def CLIP2FL():
    ...
    if args.dataset == 'cifar10':
        ...
        # STL-10 경로를 절대 경로로 변환
        args.path_stl10 = os.path.abspath(args.path_stl10)
        
        # 실제 데이터가 있는 경로 확인 (프로젝트 내 data 폴더 우선)
        ...
        # 프로젝트 내 데이터 경로에 데이터가 있으면 그 경로 사용
    ...
    # text_features 길이와 num_classes 일치 확인
